...
Partial Class POExporttoExcel
    ...
    Public Function FillVendor() As DataTable
        ...
        Dim ssql As String = ""
        Dim dt As New DataTable()
        ssql = "SELECT * FROM [VendorInformation] where   CompanyID=@f0 and DivisionID=@f1 and DepartmentID=@f2 order by VendorName ASC"
        Dim da As New SqlDataAdapter
        Dim com As SqlCommand
        com = New SqlCommand(ssql, connec)
        Try
    ...
    Public Function GetReportData() As DataTable
        ...
        Dim constr As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim connec As New SqlConnection(constr)
        Dim ssql As String = "[dbo].[POExcelImportReportNewBeta]"
        ...
        com.Parameters.Add(New SqlParameter("@ProductVendorCode", SqlDbType.NVarChar, 36)).Value = drpProductVendor.SelectedValue
        ...
        Session("txtDeliveryDateTO") = txtDeliveryDate.Text
        Session("cmblocationid") = cmblocationid.SelectedValue
        Session("ProductVendorCode") = drpProductVendor.SelectedValue
