...
Partial Class RequisitionOrder
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Me.CompanyID = "DierbergsMarkets,Inc63017" Then
            Response.Redirect("RequisitionOrderDB.aspx")
        End If
    ...
    Public Function LoadItemsData(ByVal ItemCategoryID As String, Optional ItemFamilyID As String = "") As DataTable
    ...
    Public Function LoadItemsDataDierbergsMarkets(ByVal ItemCategoryID As String, Optional ItemFamilyID As String = "") As DataTable
        Dim connec As New SqlConnection(constr)
        Dim ssql As String = ""
        Dim dt As New DataTable()
        ssql = "select  ItemID,[InventoryItems].UnitPrice , [InventoryItems].UnitsPerBox ,VendorID from [InventoryItems] where  ItemID <> 'DEFAULT' AND CompanyID=@CompanyID AND DivisionID=@DivisionID AND DepartmentID=@DepartmentID AND  InventoryItems.IsActive=1 and InventoryItems.WireServiceIdAllowed = 1 "
            ...
            If Me.CompanyID = "QuickfloraDemo" Or Me.CompanyID = "DierbergsMarkets,Inc63017" Then
                If lblmasterlocation.Text <> "" And lblmasterlocation.Text <> cmblocationid.SelectedValue Then
                    ...
                    If dt_lc.Rows.Count > 0 Then
                        ...
                        End If
                        QFmail.newmailsending(emm, emaillc, "dunlapj@dierbergs.com", "", lblmail.Text, emailconent, CompanyID, DivisionID, DepartmentID)
    ...
    Public Function getitemnamePO_Requisition_Details(ByVal InLineNumber As String) As String
        Dim itemname As String = ""
        Dim Product As String = ""
        Dim sql As String = "select Product from  PO_Requisition_Details where  InLineNumber=@InLineNumber AND CompanyID=@CompanyID AND DivisionID=@DivisionID AND DepartmentID=@DepartmentID  "
        Dim myCon As New SqlConnection(ConfigurationManager.AppSettings("ConnectionString"))
        Dim myCommand As New SqlCommand(sql, myCon)
        myCommand.Parameters.AddWithValue("@CompanyID", CompanyID)
        myCommand.Parameters.AddWithValue("@DivisionID", DivisionID)
        myCommand.Parameters.AddWithValue("@DepartmentID", DepartmentID)
        myCommand.Parameters.AddWithValue("@InLineNumber", InLineNumber)
        ...
        If dt.Rows.Count <> 0 Then
            ...
            Try
                Product = dt.Rows(0)("Product").ToString()
            Catch ex As Exception
    ...
    Private Sub OrderHeaderGrid_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles OrderHeaderGrid.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If Me.CompanyID = "DierbergsMarkets,Inc63017" Or Me.CompanyID = "QuickfloraDemo" Then
                ...
                txtQ_REQ.Attributes.Add("onblur", "myFocusFunctiontotalnewqreq('" & txtQ_REQ.ClientID & "','" & txtExt_COSt.ClientID & "','" & txtInLineNumber.Text & "','" & txtQ_ORD.ClientID & "','" & txtPACK.ClientID & "','" & txtCOST.ClientID & "')")
            Else
    ...
    Private Sub btnsavechanges_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnsavechanges.Click, btnsavechangesUP.Click, btnrecalculate.Click
        txtlastchanged.Text = Date.Now
        txtlastchangedby.Text = EmployeeID
            ...
            Try
                ...
                If True Then
                    ...
                    If InLineNumber <> "0" Then
                        ...
                        ',[ProductName]=@ProductName  ,[Product]=@Product ,[QOH]=@QOH ,[DUMP]=@DUMP ,[Q_REQ]=@Q_REQ ,[PRESOLD]=@PRESOLD ,[COLOR_VARIETY]=@COLOR_VARIETY ,[REMARKS]=@REMARKS ,[Q_ORD]=@Q_ORD ,[PACK]=@PACK ,[COST]=@COST ,[Ext_COSt]=@Ext_COSt ,[Vendor_Code]=@Vendor_Code,[Q_Recv]=@Q_Recv ,[ISSUE]=@ISSUE 

                        qry = "Update PO_Requisition_Details SET  [ProductName]=@ProductName,[ShipDate]=@ShipDate ,[Location]=@Location ,[Type]=@Type ,[HDStatus]=@HDStatus " _
                        ...
                        Else
                            'getitemnamePO_Requisition_Details
                            com.Parameters.Add(New SqlParameter("@ProductName", SqlDbType.NVarChar, 50)).Value = getitemnamePO_Requisition_Details(InLineNumber)
                        End If
    ...
    Private Sub LoadProductList(ByVal ItemCategoryID As String, Optional ItemFamilyID As String = "")
    ...
    Private Sub RequisitionOrder_PreRenderComplete(sender As Object, e As EventArgs) Handles Me.PreRender
    ...
    Private Sub dvsave_Load(sender As Object, e As EventArgs) Handles dvsave.Load
