...
Partial Class emailMT
    ...
    Private Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsNothing(Request.QueryString("TransferNumber")) Then
    ...
    Public Sub EmailNotifications(ByVal TransferNumber As String)
        ...
        Dim TansferFromLocation As String = ""


        Dim TransferToLocation As String = ""
        Dim TransferByEmployee As String = ""

        Dim ApprovedByEmployee As String = ""
        ...
        If (dtPO.Rows.Count <> 0) Then
            ...
            Try
                TransferDate = dtPO.Rows(0)("TransferDate")
            Catch ex As Exception

            End Try

            Try
                TansferFromLocation = FillLocationName(dtPO.Rows(0)("TansferFromLocation"))
            Catch ex As Exception

            End Try
            Try
                TransferToLocation = FillLocationName(dtPO.Rows(0)("TransferToLocation"))
            ...
            Try
                TransferByEmployee = Employeename(dtPO.Rows(0)("TransferByEmployee"))
            Catch ex As Exception
            ...
            Try
                ApprovedByEmployee = Employeename(dtPO.Rows(0)("ApprovedByEmployee"))
            Catch ex As Exception

            End Try

            Try
                TotalItemsTransfer = dtPO.Rows(0)("TotalItemsTransfer")
            Catch ex As Exception
        ...
        StrBody.Append("<td  ><b> Transfer Number </b></td>")
        ...
        StrBody.Append("<td  ><b> Transfer Date </b></td>")

        StrBody.Append("<td  ><b> " & TransferDate & " </b></td>")

        StrBody.Append("</tr>")

        StrBody.Append("<tr    align='center'>")

        StrBody.Append("<td  ><b> Tansfer From Location </b></td>")

        StrBody.Append("<td  ><b> " & TansferFromLocation & " </b></td>")

        StrBody.Append("</tr>")

        StrBody.Append("<tr    align='center'>")

        StrBody.Append("<td  ><b> Transfer To Location </b></td>")

        StrBody.Append("<td  ><b> " & TransferToLocation & " </b></td>")

        StrBody.Append("</tr>")

        StrBody.Append("<tr    align='center'>")

        StrBody.Append("<td  ><b> Transfer By Employee </b></td>")

        StrBody.Append("<td  ><b> " & TransferByEmployee & " </b></td>")

        StrBody.Append("</tr>")

        StrBody.Append("<tr    align='center'>")

        StrBody.Append("<td  ><b> Approved By Employee </b></td>")

        StrBody.Append("<td  ><b> " & ApprovedByEmployee & " </b></td>")

        StrBody.Append("</tr>")

        StrBody.Append("<tr    align='center'>")

        StrBody.Append("<td  ><b> Total Items  </b></td>")

        StrBody.Append("<td  ><b> " & TotalItemsTransfer & " </b></td>")
        ...
        StrBody.Append("<td ><b>Transfer Qty</b></td>")

        StrBody.Append("<td  ><b>Received Qty</b></td>")

        StrBody.Append("<td  ><b>Unit Price</b></td>")

        StrBody.Append("<td  ><b>Addional Notes</b></td>")
        ...
        For n = 0 To ds.Tables(0).Rows.Count - 1
            ...
            tr = tr & "<td>" & ds.Tables(0).Rows(n)("TransferQty") & "</td>"
            tr = tr & "<td>" & ds.Tables(0).Rows(n)("ReceivedQty") & "</td>"
            tr = tr & "<td>$" & ds.Tables(0).Rows(n)("MTPrice") & "</td>"
            tr = tr & "<td>" & ds.Tables(0).Rows(n)("AddtionalItemNotes") & "</td>"
        ...
        StrBody.Append("<td  ><b> " & "<a  target='_blank'  href='https://reports.quickflora.com/reports/scripts/MTReport.aspx?CompanyID=" & Me.CompanyID & "&DivisionID=" & Me.DivisionID & "&DepartmentID=" & Me.DepartmentID & "&TransferNumber=" & TransferNumber & "' >Click to Open</a>" & " </b></td>")
        ...
        OrderPlacedSubject = "Inventory Transfer # " & TransferNumber & " From Location " & TansferFromLocation & " To Location " & TransferToLocation
    ...
    Public Function GetInventoryTransferItemsList(ByVal TransferNumber As String) As Data.DataSet
        ...
        qry = "select  *  from [InventoryTransferDetail]   where [InventoryTransferDetail].TransferNumber =@f31 and CompanyID = @f0 AND DivisionID = @f1 AND DepartmentID = @f2 "
        'Values (@f00,@f0,@f1,@f2,@f3,@f31,@f32,@f311,@f322)
        Dim com As SqlCommand
        com = New SqlCommand(qry, connec)
        Try
    ...
    Protected Sub btnmore_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnmore.Click
        ...
        Response.Redirect("emailMT.aspx?TransferNumber=" & Request.QueryString("TransferNumber"))
    End Sub
