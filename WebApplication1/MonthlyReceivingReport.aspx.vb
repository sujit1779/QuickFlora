...
Partial Class MonthlyReceivingReport
    ...
    Public Sub GetMonthList(ByVal SetCurruntMonth As Boolean)
    ...
    Public Sub GetProductVendor()
        ...
        Dim ssql As String = ""
        Dim dt As New DataTable()
        ssql = "SELECT  distinct InventoryItems.ProductVendor FROM [InventoryItems] where   CompanyID=@f0 and DivisionID=@f1 and DepartmentID=@f2 AND ISNULL(InventoryItems.ProductVendor,'') <> '' Order by ProductVendor "
    ...
    Public Function GetReportData() As DataTable
        ...
        Dim constr As String = ConfigurationManager.AppSettings("ConnectionString")
        Dim connec As New SqlConnection(constr)
        Dim ssql As String = "[dbo].[ReceivingreportbyMonth]"

        txtDeliveryDate.Text = drpmonth.SelectedValue & "/01" & "/2020"

        If drpmonth.SelectedValue = "2" Then
            txtDeliveryDateTO.Text = drpmonth.SelectedValue & "/28" & "/2020"
        Else
            txtDeliveryDateTO.Text = drpmonth.SelectedValue & "/30" & "/2020"
        End If
        ...
        If drpstatus.SelectedValue = "" Then
            com.Parameters.Add(New SqlParameter("@bydate", SqlDbType.NVarChar, 36)).Value = "3"
            Session("bydate") = "3"
        End If
