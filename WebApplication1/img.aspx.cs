...
public partial class img :  System.Web.UI.Page
{
    ...
    public string  UpdateHomeImage(string companyID, string divisionID, string departmentID, string ItemName)
    {
        string imgname = "";
        using (SqlConnection connection = new SqlConnection(connectionString))
        {
            using (SqlCommand command = new SqlCommand("Select ISNULL(ThumbnailImage ,'noimagenew.png') AS ThumbnailImage FROM [Enterprise].[dbo].[InventoryItems]    Where ItemName=@ItemName AND  [CompanyID] =@CompanyID AND [DivisionID] = @DivisionID AND [DepartmentID] =@DepartmentID  ", connection))
            {
