...
Partial Class RequestReportB
    ...
    Public Function GetReportData() As DataTable
        ...
        ssql = ssql & " ,[Q_ORD] ,[PACK] , [PO_Requisition_Details].[COST] As 'COST' ,[Ext_COSt] ,[Buyer] ,[PO_Requisition_Details].[Status], PO_Requisition_Header.Location"
        ssql = ssql & "  "
        ...
        If optBought.Checked And chkWithOther.Checked = False Then
            ssql = ssql & " And (ISNULL([PO_Requisition_Details].PONO,'') <> '' OR  PO_Requisition_Details.Status = 'Bought'  OR  PO_Requisition_Details.Status = 'Bought Wed'   OR  PO_Requisition_Details.Status = 'Pending Email' )"
        End If

        If optBought.Checked And chkWithOther.Checked Then
            ssql = ssql & " And (ISNULL([PO_Requisition_Details].PONO,'') <> '' OR  PO_Requisition_Details.Status = 'Bought'   OR  PO_Requisition_Details.Status = 'Bought Wed'   OR  PO_Requisition_Details.Status = 'Pending Email'   OR PO_Requisition_Details.Status = 'With-Other' )"
        End If
