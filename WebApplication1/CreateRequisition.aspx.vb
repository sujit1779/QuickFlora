...
Partial Class CreateRequisition
    ...
    Public Function LoadItemsData(ByVal ItemCategoryID As String) As DataTable
        ...
        ssql = "select  ItemID,[InventoryItems].UnitPrice , [InventoryItems].UnitsPerBox ,VendorID,UOM,ItemUOM,wholesalePrice from [InventoryItems] where  ItemID <> 'DEFAULT' AND CompanyID=@CompanyID AND DivisionID=@DivisionID AND DepartmentID=@DepartmentID AND  InventoryItems.IsActive=1 and InventoryItems.WireServiceIdAllowed = 1 "
    ...
    Public Sub SetOrderProductEmptyData()
        ...
        dt.Columns.Add(New DataColumn("WHOPrice"))
    ...
    Public Sub SetOrderData()
        ...
        CommandText = CommandText & " ( Select SUM(round(convert(money,  CASE WHEN  Isnumeric(Q_REQ)  = 1  THEN  Q_REQ ELSE '0' END) * convert(money,  CASE WHEN  Isnumeric(WHOPrice)  = 1  THEN  WHOPrice ELSE '0' END) ,2)   ) "
        ...
        Try
            ...
            If dt.Rows.Count <> 0 Then
                ...
                Catch ex As Exception
                    lblsavealert.Text = lblsavealert.Text & " " & ex.Message
                    dvsavealert.Visible = True
                End Try
    ...
    Public Function saveall() As Boolean
        ...
        lblsave.Text = "Requisition <strong>#" & txtoderno.Text & "</strong> saved Successfully Total=" & txttotal.Text
        dvsave.Visible = True
    ...
    Private Sub btnaddnew_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnaddnew.Click
        ...
        For Each row As GridViewRow In OrderHeaderGrid.Rows
            ...
            Dim txtUOM As TextBox = row.FindControl("txtUOM")
            Dim txtWHOPrice As TextBox = row.FindControl("txtWHOPrice")
        ...
        If OrderNo <> "" Then
            qry = "insert into PO_Requisition_Details ([CompanyID] ,[DivisionID] ,[DepartmentID] ,[OrderNo] ,[Product] ,[QOH] ,[DUMP] ,[Q_REQ] ,[PRESOLD] ,[COLOR_VARIETY] ,[REMARKS] ,[Q_ORD] ,[PACK] ,[COST] ,[Ext_COSt] ,[Vendor_Code] ,[Buyer] ,[Status] ,[Q_Recv] ,[ISSUE],[PONO],UOM,WHOPrice)" _
             & " values(@CompanyID ,@DivisionID ,@DepartmentID ,@OrderNo ,@Product ,@QOH ,@DUMP ,@Q_REQ ,@PRESOLD ,@COLOR_VARIETY ,@REMARKS ,@Q_ORD ,@PACK ,@COST ,@Ext_COSt ,@Vendor_Code ,@Buyer ,@Status ,@Q_Recv ,@ISSUE,'',@UOM,0) "
            ...
            com.Parameters.Add(New SqlParameter("@UOM", SqlDbType.NVarChar)).Value = ""
    ...
    Private Sub OrderHeaderGrid_RowDataBound(ByVal sender As Object, ByVal e As GridViewRowEventArgs) Handles OrderHeaderGrid.RowDataBound
        ...
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            Dim txtUOM As TextBox = e.Row.FindControl("txtUOM")
            Dim txtWHOPrice As TextBox = e.Row.FindControl("txtWHOPrice")
            ...
            Dim txtQ_REQTotal As TextBox = e.Row.FindControl("txtQ_REQTotal")

            Try
                Dim qtyrqtotal As Decimal = 0
                ...
                End Try
                Try
                    price = txtWHOPrice.Text
                Catch ex As Exception
                    price = 0
                End Try
                qtyrqtotal = qty * price
                txtQ_REQTotal.Text = Format(qtyrqtotal, "0.00")
                txtWHOPrice.Text = Format(price, "0.00")
            Catch ex As Exception
            ...
            ' txtWHOPrice.Enabled = False

            If txtStatus.Text <> "No Action" Then
                If txtStatus.Text <> "DateChanged" Or True Then
            ...
            txtProduct.Attributes.Add("onKeyUp", "SendQuery2(this.value,this,'" & txtInLineNumber.Text & "','" & txtPACK.ClientID & "','" & txtCOST.ClientID & "','" & txtVendor_Code.ClientID & "','" & txtUOM.ClientID & "','" & txtWHOPrice.ClientID & "')")
            ...
            txtQ_REQ.Attributes.Add("onblur", "myFocusFunctiontotalnew('" & txtQ_REQTotal.ClientID & "','" & txtInLineNumber.Text & "','" & txtQ_REQ.ClientID & "','" & txtWHOPrice.ClientID & "')")

            txtWHOPrice.Attributes.Add("onfocus", "myFocusFunction(this)")
            ' txtQ_REQ.Attributes.Add("onblur", "Saveitem(this,'" & txtInLineNumber.Text & "','Q_REQ','" & txtQ_REQ.Text & "')")
            txtWHOPrice.Attributes.Add("onblur", "myFocusFunctiontotalnew('" & txtQ_REQTotal.ClientID & "','" & txtInLineNumber.Text & "','" & txtQ_REQ.ClientID & "','" & txtWHOPrice.ClientID & "')")
    ...
    Public Function savechages() As Boolean
        ...
        For Each row As GridViewRow In OrderHeaderGrid.Rows
            Try
                ...
                Dim txtPACK As TextBox = row.FindControl("txtPACK")
                Dim txtCOST As TextBox = row.FindControl("txtCOST")
                Dim txtExt_COSt As TextBox = row.FindControl("txtQ_REQTotal")
                Dim txtVendor_Code As TextBox = row.FindControl("txtVendor_Code")
                Dim txtBuyer As Label = row.FindControl("txtBuyer")
                ...
                If True Then
                    ...
                    If InLineNumber <> "0" Then
                        ...
                        com.Parameters.Add(New SqlParameter("@InLineNumber", SqlDbType.BigInt)).Value = InLineNumber
                        com.Parameters.Add(New SqlParameter("@ItemUOM", SqlDbType.NVarChar)).Value = txtUOM.Text
                        com.Parameters.Add(New SqlParameter("@WHOPrice", SqlDbType.NVarChar)).Value = txtWHOPrice.Text
                        ...
                        com.Parameters.Add(New SqlParameter("@ShipDate", SqlDbType.NVarChar)).Value = txtshipdate.Text
                        com.Parameters.Add(New SqlParameter("@Type", SqlDbType.NVarChar)).Value = drpType.SelectedValue
                        com.Parameters.Add(New SqlParameter("@ProductName", SqlDbType.NVarChar)).Value = getitemname(txtProduct.Text)
    ...
    Private Sub LoadProductList(ByVal ItemCategoryID As String)
        savechages()
        saveall()
        ...
        Else
            ...
            For n = 0 To dtLoadItemsData.Rows.Count - 1
                ...
                dr("UOM") = dtLoadItemsData.Rows(n)("ItemUOM")
                dr("WHOPrice") = dtLoadItemsData.Rows(n)("wholesalePrice")
                dr("QOH") = String.Empty
                dr("DUMP") = String.Empty
                dr("Q_REQ") = String.Empty
                dr("PRESOLD") = String.Empty
