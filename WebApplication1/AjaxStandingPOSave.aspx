...
<script runat="server">
    ...
    Public Function UpdateOrderTotal(ByVal RowNumber As String) As Boolean
        ...
        Dim ssql As String = ""
        Dim dt As New DataTable()
        ssql = "SELECT OrderNo FROM [PO_Standing_Requisition_Details] where  InLineNumber = @InLineNumber and CompanyID=@f0 and DivisionID=@f1 and DepartmentID=@f2  "
        Dim da As New SqlDataAdapter
        Dim com As SqlCommand
        com = New SqlCommand(ssql, connec)
        Try
    ...
    Public Function Updatetotal(ByVal OrderNo As Integer, ByVal Total As Decimal) As Boolean
        ...
        qry = "Update PO_Standing_Requisition_Header SET   TotalAmount=@TotalAmount   Where  OrderNo = @OrderNo and CompanyID=@f0 and DivisionID=@f1 and DepartmentID=@f2  "
    ...
    Public Function GetOrderTotal(ByVal OrderNo As String) As Decimal
        ...
        Dim ssql As String = ""
        Dim dt As New DataTable()
        ssql = "SELECT SUM(CONVERT(Money, Ext_COSt)) FROM [PO_Standing_Requisition_Details] where  ISNUMERIC(Ext_COSt) = 1 and  OrderNo = @OrderNo and CompanyID=@f0 and DivisionID=@f1 and DepartmentID=@f2  "
        Dim da As New SqlDataAdapter
        Dim com As SqlCommand
        com = New SqlCommand(ssql, connec)
        Dim Total As Decimal = 0
    ...
    Public Function SelectOrderProductData(ByVal RowNumber As String) As DataTable
        ...
        Dim ssql As String = ""
        Dim dt As New DataTable()
        ssql = "SELECT * FROM [PO_Standing_Requisition_Details] where  InLineNumber = @InLineNumber and CompanyID=@f0 and DivisionID=@f1 and DepartmentID=@f2  "
        Dim da As New SqlDataAdapter
        Dim com As SqlCommand
        com = New SqlCommand(ssql, connec)
        Try
    ...
    Public Function InsertPO_OrderDetails(ByVal RowNumber As String) As Integer
        ...
        If OrderNo <> "" Then
            qry = "insert into PO_Standing_Requisition_Details ([CompanyID] ,[DivisionID] ,[DepartmentID] ,[OrderNo] ,[Product] ,[QOH] ,[DUMP] ,[Q_REQ] ,[PRESOLD] ,[COLOR_VARIETY] ,[REMARKS] ,[Q_ORD] ,[PACK] ,[COST] ,[Ext_COSt] ,[Vendor_Code] ,[Buyer] ,[Status] ,[Q_Recv] ,[ISSUE])" _
        ...
        Dim ssql As String = ""
        Dim dt As New DataTable()
        ssql = "SELECT MAX(InLineNumber) FROM [PO_Standing_Requisition_Details] where  Product=@Product and  OrderNo = @OrderNo and CompanyID=@f0 and DivisionID=@f1 and DepartmentID=@f2  "
        Dim da As New SqlDataAdapter
    ...
    Public Function InsertOrder_AjaxBinding(ByVal RowNumber As Integer, ByVal name As String, ByVal value As String) As Boolean
        ...
        qry = "Update PO_Standing_Requisition_Details SET  " & name & " =@value,Buyer=@Buyer   Where InLineNumber = @RowNumber"
        If name.ToLower = "buyer" Then
            qry = "Update PO_Standing_Requisition_Details SET  " & name & " =@value    Where InLineNumber = @RowNumber"
        End If
        'Values (@f00,@f0,@f1,@f2,@f3,@f31,@f32,@f311,@f322)
        Dim com As SqlCommand
        com = New SqlCommand(qry, connec)
    ...
    Public Sub addchangehistory(ByVal RowNumber As Integer, ByVal name As String, ByVal value As String)
        ...
        Logchangehistory("", EmployeeID, "PO_Standing_Requisition_Details", name, value, OrderNo, RowNumber, type)
